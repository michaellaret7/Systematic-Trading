...
A comprehensive guide for creating effective Agent Skills that extend Codex's capabilities with specialized knowledge, workflows, and tools.

## Overview

Agent Skills are filesystem-based resources that provide Codex with domain-specific expertise. Skills use **progressive disclosure** to efficiently manage context:
...
- What procedural knowledge does Codex lack for this task?
...
This creates `~/.Codex/skills/<skill-name>/` by default (Codex's user-level skills folder).

Options:
- `--path <dir>` - Create skill in a different location (e.g., `--path ./project/.Codex/skills`)
...
The frontmatter is critical - it determines when Codex uses your skill.
...
- Include only information Codex doesn't already know
- Prefer concise examples over verbose explanations
- Reference bundled files explicitly so Codex knows they exist
...
The zip file can be:
- Uploaded to Codex.ai via Settings > Features
- Used with Codex API via the skills parameter
- Installed in Codex at `~/.Codex/skills/` or `.Codex/skills/`
...
1. **Vague descriptions** - Codex won't know when to trigger the skill
2. **Including obvious information** - Codex already knows how to code, explain concepts, etc.
...
1. **Observe** Codex using the skill on real tasks
...
Ask Codex to help improve skills:
